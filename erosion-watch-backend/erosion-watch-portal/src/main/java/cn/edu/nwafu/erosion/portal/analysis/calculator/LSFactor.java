...
public class LSFactor {
    public static double calculateLS(double slopeAngleDegrees, double slopeLength) {
        ...
        double L = Math.pow(Math.sqrt(slopeLength / 22.1), m);
        double S = calculateS(slopeAngleDegrees);
        return L * S;
    }

    private static double calculateBeta(double thetaDegree) {
        return (Math.sin(thetaDegree) / 0.0896) / (3.0 * Math.pow(Math.sin(thetaDegree), 0.8) + 0.56);
    }

    private static double calculateS(double thetaDegree) {
        if (thetaDegree < 5.14) {
            return 10.8 * Math.sin(thetaDegree) + 0.03;
        } else {
            return Math.pow(Math.sin(thetaDegree) / 0.0896, 0.6);
        }
    }

    public static void main(String[] args) {
        double slopeAngleDegrees = 9; // 坡度角度示例值
